...
export function DirectorRunCard({ run, campaign, busy, action }: { run: DirectorRun; campaign: Campaign; busy: boolean; action: (body: unknown) => Promise<unknown> }) {
  const [allowance, setAllowance] = useState(Math.min(30, run.reels * (2 + run.stillsPerReel) + 2));
  ...
  return <Card className="my-5 border-primary/25 bg-card/70">
    ...
    <CardContent className="space-y-4">
      ...
      {run.status === "awaiting_approval" && <div className="space-y-4 rounded-xl border border-primary/20 p-4"><h4 className="flex items-center gap-2 text-sm font-medium"><Film size={16} />Ready for production</h4><p className="text-xs leading-relaxed text-muted-foreground">{campaign.imageProvider === "codex" ? "OpenAI account images" : "Kie.ai images"} → {motionModelSummary(motionModel(run.videoModel))}. The selected clips supply the motion reference. The allowance includes anchors, videos, stills, and corrective generations.</p>
        {!campaign.identity && <p className="text-xs text-destructive">Select an identity above, or switch to Plan & create to build and save one. Sources will be reassessed against the saved identity.</p>}
        <Field><FieldLabel htmlFor={`allowance-${run.id}`}>Maximum media generations</FieldLabel><input id={`allowance-${run.id}`} type="number" min={2} max={30} value={allowance} onChange={e => setAllowance(Number(e.target.value))} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" /></Field>
        <Field><FieldLabel htmlFor={`motion-estimate-${run.id}`}>Estimated cost per motion video, USD</FieldLabel><input id={`motion-estimate-${run.id}`} type="number" min="0.01" step="0.01" placeholder={campaign.budget?.maxEstimatedUsd != null ? "Required for your dollar planning limit" : "Optional · based on the selected clip length"} value={motionEstimate} onChange={e => setMotionEstimate(e.target.value)} className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" /></Field>
        ...
        <Button disabled={busy || !reuse || !campaign.identity} onClick={() => action({ action: "director-approve", runId: run.id, maxGenerations: allowance, ...(motionEstimate ? { motionEstimateUsd: Number(motionEstimate) } : {}), referenceReuseConfirmed: true })}><Play data-icon="inline-start" />Approve bounded production</Button>
      </div>}
