...
import { listIdentityAssets } from "../../guest/identityAssets";
import { clipLimits, motionModel, motionModelBehavior, motionModelSummary } from "./motionModels";
export async function accountJSON(prompt: string, images: string[] = [], webSearch = false, provider: AccountProvider = "codex", tier: ModelTier = "reasoning") {
  const response = await accountPlanner(provider)(new NextRequest("http://localhost/api/assistant", { method: "POST", body: JSON.stringify({ messages: [{ role: "user", content: prompt }], imageUrls: images }) }), { webSearch, maxImages: 8, tier });
...
{"tool":"propose_identity","name":"influencer name","dna":"stable visual identity: face, hair, body, age range (adult), skin, signature features","personality":"voice and personality","direction":"wardrobe, styling and setting that fit the SELECTED footage","reason":"..."}
...
Search queries should vary if downloads fail. You may use the user's source URLs immediately. Retrieve before inspection, inspect before selection, select before generation. Reject unsuitable sources and choose others. Select ${limits.minSeconds}–${limits.maxSeconds} seconds avoiding cuts. Select ALL requested sources before the first generation proposal; production approval freezes those sources. ${run.identity?.references.length ? "An influencer is saved: never call propose_identity." : `No influencer is saved yet. After ALL requested sources are selected, call propose_identity exactly once. The bar is a top-tier Instagram creator: striking, magnetic, editorial beauty that stops the scroll, with a specific memorable face (not generic), polished hair and makeup, and an on-trend, flattering, intentional wardrobe that reads as fashion/lifestyle rather than plain athletic basics. Fit the selected clips' framing, camera height, lighting and range of motion so motion transfer holds, but never copy the source dancer's identity or dress her down to match the source; elevate the styling instead. Follow any look, age, style or vibe the objective or brief specifies; swimwear, beach, fitness and nightlife content is ordinary SFW influencer material, so do not sanitize it or add hedges like "opaque" or "modest". Give her a distinctive name and look: never reuse the names, faces or feature sets of the existingInfluencers in state, and vary ethnicity, features and hair rather than defaulting to one persona. Adults only, no nudity. ${run.identityProposal ? "The proposal exists; candidates are produced automatically and the user picks one. Do not propose again." : ""}`} Inspect at least two successfully retrieved candidates when search offers alternatives; compare persona fit, clarity, transfer risk and verified metrics. Never use a source marked unsuitable.
An anchor is a new image of the TARGET influencer with pose/framing/style compatible with the selected motion clip; retain identity while varying wardrobe per content pack. Use target identity references, not the source person's face. Review the anchor before motion or stills. Motion uses ${motionModelSummary(model)}. ${motionModelBehavior(model)} Match stills to the approved anchor and the scene the motion video will show. Review every output before finish (influencer candidates are chosen by the user and are not reviewed). On a failed review, improve the prompt, regenerate the anchor, or change source. Don't keep retrying the same prompt. Max two retries per kind/source. A tool may fail: adapt rather than claiming success. Never declare an asset generated or inspected until its real tool result exists. Do not publish.
...
State: ${JSON.stringify({ existingInfluencers: run.identity?.references.length ? undefined : listIdentityAssets().slice(0, 40).map(i => ({ name: i.name, look: (i.basePrompts[0] ?? "").slice(0, 140) })), sources: run.sources.map(source => ({ ...source, downloadUrl: undefined })), jobs: run.jobs, assets: assets.map(a => ({ id: a.id, sourceId: a.sourceId, kind: a.productionKind ?? a.kind, title: a.title, automatedReview: a.automatedReview, text: a.text })), recentEvents: run.events.slice(-15), decisionCount: run.decisionCount })}
