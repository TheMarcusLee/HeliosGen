 ...
 * database (uploaded as JSON, e.g. an App Promo Factory / Viral Reel Creator
 * export) and their images stay remote until a template is used, when the
 * image is mirrored into local media and an identity asset is created.
 */
...
export function deleteTemplate(id: string) { table().prepare("DELETE FROM identity_templates WHERE id = ?").run(id); }
 ...
 */
export async function materializeTemplate(id: string, mirror: (url: string) => Promise<string> = (url) => storeMedia(url, "identity-templates", "image")): Promise<IdentityAsset> {
