...
function createServer(): McpServer {
  ...
  server.registerTool("helios_campaign_action", {
    ...
    description: "Use the same campaign service as the chat UI. Request forms: {action:'message',text} plans using the configured text model (billable); {action:'edit-plan',messageId,identityDraft?,steps:[{title,prompt,look}]} edits the latest plan's brief before it starts (same step count); {action:'start',messageId} authorizes the listed media generations, which run in parallel where steps are independent; {action:'advance'} submits/polls the next authorized step; {action:'identity',identityId} selects a saved identity (null clears); {action:'save-identity',assetId} saves an approved reference as an influencer; {action:'review',assetId,review:'approved'|'rejected'|'pending'}; {action:'run',runId,operation:'pause'|'resume'|'stop'|'retry'} (retry re-queues the failed steps of an errored plan run within the budget); {action:'settings',title?,model?,imageModel?,imageProvider?:'codex'|'antigravity'|'kie',videoModel?,motionModel?,referenceUrls?} (model 'codex-account' or 'antigravity-account' selects the connected OpenAI or Google account for planning and director reasoning; motionModel: default reference-video model for director runs). Show the plan and obtain user authorization before start. Additional actions: {action:'memory',memory:{brand,brief,audience,voice,constraints,products:[{name,description,url}]}}; {action:'budget',budget:{maxGenerations,maxEstimatedUsd,imageEstimateUsd,videoEstimateUsd}} (nullable dollar fields); {action:'discover',query,source?:'tiktok'|'youtube',region?}; {action:'select-trend',trendId,selected}; {action:'add-trend',title,url,notes}; {action:'revise-text',assetId,text}; {action:'draft-post',draft:{assetIds,caption,accountIds,scheduledAt}}; {action:'queue-post',postId}; {action:'cancel-post',postId}. Director actions: {action:'director-start',objective,sourceUrls?:[],reels?:1,stillsPerReel?:1,videoModel?:'any Kie.ai model accepting a reference video, default kling-3.0-motion-control',searchProvider?:'tiktok'|'web' (tiktok = built-in live browser search, free; web = OpenAI account web search)}; {action:'director-approve',runId,maxGenerations,motionEstimateUsd?,referenceReuseConfirmed?} (two stages when the campaign has no saved identity: the first approval covers only the proposed influencer's reference candidates and needs no reuse confirmation; after {action:'director-identity'} picks one, the run returns to awaiting_approval for production, which requires referenceReuseConfirmed:true); {action:'director-control',runId,operation:'pause'|'resume'|'stop'}; {action:'director-advance'}; {action:'director-identity-proposal',runId,name,dna,personality,direction} edits the proposed influencer before candidates exist; {action:'director-identity',runId,assetId} saves one agent-generated influencer candidate as the campaign identity when a run is awaiting_identity (runs without a saved identity design one to fit the selected footage); {action:'director-reply',runId,text,sourceUrls?:[]} resumes blocked/paused work with user guidance using the same allowance. Director start authorizes research, including up to four live searches; production requires approval of selected sources, footage reuse and bounded generation allowance. It retrieves real media and performs sampled-frame inspection, motion transfer and output QA. Do not describe web-index search as a complete platform trend feed. Message can include revisionOf. Dollar limits use estimates, not guaranteed bills. The server worker advances runs independently. Publishing requires explicit authorization of exact assets, caption, accounts and time before queue-post. Never retry uncertain submissions without checking the provider.",
