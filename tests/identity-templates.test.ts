...
test("templates import from an App Promo Factory export and become identities with a mirrored reference", async () => {
  ...
  const exported = [
    { id: "t1", name: "Olivia Rose", subtitle: "Soft Girl Aesthetic", category: "plus-size", gender: "female", age_range: "22-25", ethnicity: "white-caucasian", eye_color: "blue", hair_color: "dyed-custom", body_shape: "plus-size", prompt_value: "A beautiful 24-year-old plus-size woman with lilac-pink waves and a violet hairclip.", image_url: "https://example.com/olivia.png", sort_order: 34 },
    { name: "Marcus", category: "streetwear", gender: "male", prompt: "A confident 25-year-old man with a lavender fade.", imageUrl: "/generated/templates/marcus.png" },
    { name: "No prompt" },
  ...
  assert.equal(listTemplates({ gender: "male", query: "lavender fade" }).length, 1); assert.equal(listTemplates({ query: "violet hairclip" })[0].id, "t1");
  ...
  assert.match(identity.basePrompts[0], /lilac-pink/); assert.match(identity.basePrompts[1], /eye color: blue/);
